...
pub fn get_or_build_client(opts: ClientOptions) -> Client<hyper_rustls::HttpsConnector<hyper::client::HttpConnector>> {
    ...
    let client = build_hyper_client_cert(opts.clone());
    CLIENT_CACHE.insert(key, client.clone());
    client
}

pub fn build_hyper_client_cert(opts: ClientOptions) -> Client<HttpsConnector<HttpConnector>> {
...
pub fn build_hyper_client_proxy(opts: ClientOptions) -> Client<ProxyConnector<HttpsConnector<HttpConnector>>> {
    let timeout_duration = Duration::from_millis(1000);
...
pub fn build_hyper_client_normal() -> Client<HttpsConnector<HttpConnector>> {
    let timeout_duration = Duration::from_millis(1000);
